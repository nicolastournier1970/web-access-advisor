...
> **Status**: **COMPLETE** (2026-07-09) on branch `refactor` — all 8 phases done. Final verification: 517 tests green (shared 17, engine 369 incl. real-browser smokes + the auth-v2 gate, api 27, web 71, cli 33) + parity 100% on the golden session. v1 recoverable via the `v1-legacy` tag or `main`.
...
- [x] **Phase 8 — Docs, CLI, CI** (commit `eb67c67`): six docs verified against source (architecture, auth-flows, api-reference + real OpenAPI export, development, testing) with 10 doc/code discrepancies reconciled; fresh `@waa/cli` (parseArgs, analyze/replay/sessions, pause-aware replay auto-continue, 33 tests + real fixture smoke); GitHub Actions CI (ubuntu+windows matrix, full engine suite on Linux, zod-major guard). Fixes: double auth-segment now 409, fixture server default port 4310.
